...
```{r, message = FALSE, label = "COOL", fig.cap = " This shows a single point for each station in Seattle."}
...
```{r echo=FALSE, message=FALSE, label = "COOLZOOM", fig.cap="Here is a more zoomed in look of the stations in Seattle."}
...
```{r, echo = FALSE, label = "SUPER", fig.cap="This shows how many bikes can be parked at each station."}
...
Well that’s weird, some of the stations have a dock count of 0 shown in Figure \@ref(fig:SUPER). I’m assuming they didn’t start that way. Let’s calculate the change in dock count from station installation to August 31, 2016 and plot it on a map.
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, label = "WOW", fig.cap= "This is shows the stations that took away bike docks."}
ggmap(mymap) + 
...
Wow! Looks like quite a few stations took away bike docks and none gained any. Perhaps those stations weren’t being used very frequently. We’ll have to look at that a bit later in Figure \@ref(fig:WOW).
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, label="WOAH", fig.cap = "This shows the current size of each station."}
ggmap(mymap) + 
  geom_point(aes(x = long, y = lat, size = factor(dock_change), color= factor(dock_change)),
                          data = dock_change, alpha = 0.8) +  
  guides(color = guide_legend(title = "Change"), size = guide_legend(title = "Change")) +
...
```{r, echo = FALSE, message = FALSE, label = "NOISE", fig.cap = "Shown are the number of rides per day."}
trip_2 %>% 
  group_by(start_date) %>%
  summarize(N = n()) %>%
...
Hmm, grouping by day is a little noisy shown in Figure \@ref(fig:NOISE). Perhaps we should try by month?
...
```{r, label = "GREAT", fig.cap="This shows the number trips taken per month sorted by season."}
start_date_ym %>%
  group_by(ym, Season) %>%
  summarize(N = n()) %>%
...
```{r, label = "NICE", fig.cap = "This shows the average trip duration in minutes."}
Trip_Duration_Month %>%
...
There’s surprisingly not a huge range in trip durations here in Figure \@ref(fig:NICE).
...
```{r, label= "FANTASTIC", fig.cap="This shows the consistency across the seasons for the number of trips."}
trip_seasons %>%
  group_by(wd, Season) %>%
  summarize(N = n()) %>%
...
So it looks like usage is relatively consistent across seasons, at least as far as the number of trips are concerned shown in Figure \@ref(fig:FANTASTIC).
...
```{r, label = "SWEET", fig.cap="Shown are the time of day during the week that people are using most."}
start_date_ym %>%
...
Wow, looks like regardless of the season, people are commuting to/from work using this service (there’s a spike between 8 and 10 AM and another between 4 and 7 PM Monday through Friday). But the weekends seem to be popular between 10 AM and 10 PM shown in Figure \@ref(fig:SWEET).
...
```{r, label = "BOOM", fig.cap = "This shows the number of trips taken based on their member type."}
trip_seasons %>%
  filter(usertype != "") %>%
...
```{r, label = "NEAT", fig.cap = "This shows the trip duration (whether or not they go over the alloted time) based on member type."}
...
ggplot(data = start_date_ym, aes(x = tripduration_min)) +
  geom_histogram(aes(fill = usertype), color = "white") +
  xlim(0, 100) +
...
Ok, so our members are pretty good about making sure that they return their bike before they incur extra charges, but the short-term pass holders frequently go over their time limit shown in Figure \@ref(fig:NEAT). I wonder how the cost of a trip varies for members and pass holders. Let’s try to calculate the cost of a trip.
...
```{r, label = "FUN", fig.cap = "Shown are the extra fees that people are paying when they run over the time they had."}
start_date_ym %>% 
  filter(cost > 0) %>%
...
```{r, label = "WONDERFUL", fig.cap = "This shows which people have the annual Pronto membership."}
