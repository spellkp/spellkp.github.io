...
```{r echo=FALSE, message=FALSE, label = "COOLZOOM", fig.cap="something"}
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, label = "WOW"}
ggmap(mymap) + 
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, label="WOAH"}
ggmap(mymap) + 
  geom_point(aes(x = long, y = lat, size = factor(dock_change), color= factor(dock_change)),
                          data = dock_change, alpha = 0.8) +  
  guides(color = guide_legend(title = "Change"), size = guide_legend(title = "Change")) +
...
```{r, echo = FALSE, message = FALSE, label = "NOISE"}
trip_2 %>% 
  group_by(start_date) %>%
  summarize(N = n()) %>%
